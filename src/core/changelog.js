...
window.LJChangelog = [
  ...
  },
  {
    ...
    bullets: [
      'Master V40 fechada. Em uma frase: o LJ ganhou um cockpit operacional próprio, a Audiência virou Arquétipo de Vendas com motor composicional adaptativo, o Resultado do mês virou triangulação visual viva, e o Mapa da Receita foi varrido por dentro com 61 bugs únicos resolvidos. A V41 começa com base sólida pra abrir frentes novas sem dívida acumulada.',

      '**Arquitetura — Separação dos dois mundos.** A V40 entrou justamente com a divisão LJ-cliente vs LJ-operação. O cockpit operacional ganhou porta dedicada em `/admin` com app próprio, sidebar dedicada (Tenants + Snapshots), gerenciamento de clientes em grid de cards, e impersonation via JWT temporário de 2h ("Entrar como Mariano") que abre o LJ-cliente em nova aba com banner amarelo. O LJ-cliente passou a ser tenant comum mesmo pra Felipe — as abas "Administrar Lead Journey" e "Tenants" sumiram do menu Configurações. Migration `is_lj_operator` cravada pra separar semanticamente operador-do-LJ de master-do-tenant. Tabela `lj_impersonation_audit` registra cada sessão.',

      '**Cockpit operacional cresceu em 5 ondas.** Onda 2: plugins liberados por tenant. Onda 3: cobrança manual por hora (registro de horas e pagamentos). Onda 4: liberação de APIs/Integrações por tenant (catálogo + gating). Onda 5: gestão de usuários por tenant + uso da IA por tenant (modelo + custo + período de uso). Cards de tenant ganharam consolidação completa: status, plug/unplug DB próprio, criar usuários avulsos, ver custo de IA, ver integrações ativas — tudo numa tela só.',

      '**Audiência V2 virou Arquétipo de Vendas — motor composicional adaptativo.** A Sprint 1 externalizou o catálogo de Átomos com versionamento na fusão. A Sprint 2 adicionou 4 modelos operacionais novos (Atacado, Consultoria, Manufatura, Agribusiness) + 4 Átomos Refinadores. A Sprint 3 cravou o Step 5 de Confirmação + Confidence Score honesto + Catálogo de Consequências. As Sprints 4 e 5 ligaram o arquétipo aos consumidores: Card de Velocidade adaptativo + banner pra audiências legadas, RevOps Equilíbrio adaptativo + Djow lateral adaptativo. O nome "Definir Audiência" foi rebatizado pra "Definir Arquétipo de Vendas" — mais honesto com o que faz. A engine legada de Audiência + Coleta foi removida.',

      '**Wizard do Arquétipo passou por onda Leonardo completa.** Painel lateral navegável (zigzag entre passos), wizard 20% mais largo, roletas com aperto vertical pra eliminar vazios, confidence honesta com trava em 50% no fallback, incompatibilidades visíveis (14 cravadas no catálogo), banner enriquecido quando o LJ não classifica + sugestão de arquétipos próximos. Catálogo de Consequências ganhou novo arquétipo: Eventos B2B Anuais (feira, congresso, summit). Memória cravada: cadastro de arquétipo novo é via código direto, sem UI Master — Felipe pede aqui, eu adiciono no `audienceConsequencesCatalog.js`.',

      '**RevOps Resultado virou tríade visual semafórica.** Engine novo de Receita do mês cravado: Realizado · Projetado · Meta em uma régua. Cards de CAC e Vendas seguiram o padrão, fechando a tríade pura (Receita amarelo, CAC roxo, Vendas ciano — paleta semântica oficial). Hierarquia tipográfica reorganizada (Realizado como protagonista). Pulse semafórico de saúde, cor inversa no CAC quando estoura, cards encolhem 30%, linha "hoje" na régua, detector de meta absurda, edit-in-place da Meta. Polidura² Leonardo: fio conector, "—" elegante, banner enxuto, anti-colisão, threshold meta CAC, régua reescrita do zero com legenda em row. Bloco CTC + Fat. Bruto foi removido — a página é tríade pura agora.',

      '**RevOps Equilíbrio ganhou KPIs Avançados.** Nova seção com ROAS, Payback, %CAC/TM, Margem MCU%. KPIs Personalizados de fórmula livre foram removidos (substituídos pelos Avançados). Vendas Previstas passou a vir do funil das actions, não mais do campo manual legado. Centavos voltaram pros números pequenos (MCU, CAC, MSU). Polidores no Custo Fixo + Breakeven (formatação BR + folga gigante legível). Card de Ofertas redesenhado pra fim de fricção — Mix virou Participação, ticket deixou de mentir quando real divergir (Participação Plano vs Real). Cascata da DRE agora bate com EBITDA (linhas extras "depois de Deduções" voltaram a aparecer + EBITDA projetado da cascata casa com a DRE).',

      '**Demo Engenho Norte ficou completo e enxuto.** Cerveja Pilsen + Weiss + Chopp de Vinho ativáveis, RevOps & Governança populado nos 3 produtos, dashboard Checkout com vendas fictícias, Mapa da Receita + Audiência + 150 leads, vendas realizadas distribuídas nas ações, Pipeline Google Ads → S&M → CAC fechado, demo lê de Hotmart real (não mock) com auto-bootstrap das 3 tabelas, 9 endpoints de sistema mockados (console limpo + Health 9/9). Camada de diagnóstico cravada: Chopp em rota errada faz o LJ virar diagnóstico, não dashboard verde — paradigma "não fingir saúde" reforçado.',

      '**Velocity destravada pra produto via CRM.** Antes só funcionava pra checkout. Agora calcula V/C/L/T direto de `lj_rd_deals`. Pulso da Receita passou a falar a mesma língua do card de Velocidade pra produto CRM (Vendas/TM/CAC vêm da fonte certa quando salesChannel="crm"). Nova action `Actions.populateDemoCrmDeals(productId)` emula 500 deals em pipeline B2B realista (50 won, 150 pipeline, 300 lost) distribuídos em 6 meses, com perfis variados (distribuidoras, hotéis, restaurantes, bares, eventos). Endpoint de detecção do demo passou a cobrir CRM além do Checkout. Híbrido ainda fica pra onda futura.',

      '**Endpoints admin pra resgate e migração de tenant.** Action master `Actions.resetProductPristine(productId)` zera um produto pra estado mínimo (mantém estrutura comercial, zera arquétipo + RevOps + Hotmart + deals). POST `/api/admin-import-tenant-state` aceita JSON de backup local e escreve direto no journey_state — útil quando `admin-restore-tenant-snapshot` não serve (owner_user_id divergente) ou quando o backup local é mais recente. O endpoint ganhou modo merge: `{ onlyProductIds: [...] }` mescla só os produtos específicos preservando o que já existe — cliente perdeu produto antigo + criou produto novo, recupera só o antigo sem perder o novo.',

      '**INCIDENTE crítico e cravado: cross-tenant overwrite Sansone.** Em 2026-06-24 Atira.Pro do Sansone foi sobrescrita porque o navegador estava logado como master no tenant Sansone com state em memória de sessão anterior em outro tenant — o auto-save mandou esse state contaminado pro servidor e ele aceitou. Bug estrutural de isolamento client-side que faltava como defesa elementar. Resolvido em 2 camadas: Camada 1 (V40.14.17) — servidor bloqueia 409 no /api/state-sync se state.user.tenantId/id diverge do JWT, cliente trata 409 cancelando retry com toast crítico. Camada 2 (V40.15.0) — boot detecta troca de identidade (lj_last_user_id no localStorage) e purga state local antes de carregar do servidor, App.save() valida tenant antes de enviar. Logout limpa marcadores pra não disparar purge falso no relogin do mesmo user. Memória cravada: [[bug_client_state_leak_between_tenants]] — nunca propor teste em cliente real sem essas guards no lugar.',

      '**Audit completo do Mapa da Receita — 6 ondas, 61 bugs únicos.** Felipe relatou: "muito bug, tela piscando, escreve 2 letras e sai, reset, fecha sozinho". Saída: workflow adversarial de 143 agentes (4 inventários paralelos + 8 hunters por categoria + verify cético + síntese) → 130 candidatos → 73 confirmados (61 únicos + 12 duplicatas de mesma raiz). Onda 1 — 5 críticos (removeProductKr/Okr/Objective com confirm + cascade visível, opId guard em confirmCreateCustomKr, mind-map editor convertido pra draft controlled). Onda 2 — 19 inputs sem id+data-focus-key em batch (lei `feedback_modal_inputs_need_ids` zerada). Onda 3 — 14 bugs de reset bombs + sub-modais zumbi (helpers `_resetMapaDrafts`, `_closeAllStrategicSubModals`, `_closeStrategicChildModals`, beforeunload warning em draft em-flight). Onda 4 — 9 validações e silent failures (gates no wizard de KR, decimais funcionando, State.normalize valida selectedXId contra coleção real, 11 modais transitentes em normalize). Onda 5 — 8 navegação e overlap (Esc stack ordenado, mutex sub-modais, z-index ActionEdit ajustado, openStrategicMap sem redirect, createTaskFromOkr não fecha Mapa, openActionFromMap salva contexto, openStrategicMapForCampaign aceita {zoom, activeAreaId}). Onda 6 — 8 state drift residual + race fina (opId guard em submitTaskCreation/djowProcessKrName, productId derivado da action, cleanup amplo em delete, productEditDraft clone-on-open, merge condicional de OKRs no updateActionContext).',

      '**Outros polidores significativos.** Flow Builder passou por onda Leonardo (paleta semântica, profundidade visual, hierarquia, modo Protótipo global, Spotlight com árvore, 4 atalhos de teclado/mouse, conectar em 1 click, sidebar com scrollbar escondida, pílula fantasma vira drag handle, sequência de fixes em halo/sombra/borda no card selecionado). F5 polido: sininho/badges/integrações com status correto no boot, state fields normalizados. Fix grave que voltou Configurações/Sair/Plugins/integrações a funcionar. Fix loop infinito de /api/governance-closings + fix preventivo no loadGa4Status. Fix alerta falso "ClickUp desconectado" no F5. Aba Ações: select do "Criar ação" não filtra mais a tela inteira. Wizard Audiência ganhou refinamento como passo próprio + scroll que volta pro topo. Coleta de Cacau Show ganhou dedupe contra duplicar Marketing/Vendas/CS.',

      '**Próximo capítulo — V41.x.** Audit fechado abre caminho pra atacar frentes que ficaram em backlog: Onda B do LJ 2.0 RevOps (Governança Comercial Blindada — Matriz de Alçada, Data Gates, Compelling Event), Forecast × Realizado e Eficiência de Capital adaptados pra CRM, modo Híbrido (checkout + CRM no mesmo card), provider abstraction (Hotmart/RD/Google Ads → Stripe/Pipedrive/Meta como cidadãos de 1ª classe), Score Motion Engine, RD Fase 3 (equivalências, sync LJ→RD, motor de transição global). A V41 nasce sem dívida acumulada — é hora de cravar produto novo, não mais consertar.'
    ]
  },
  {
    version: 'V40.16.4',
