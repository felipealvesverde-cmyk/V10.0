// V35.3.3 — Google Ads Dashboard (placeholder).
//
// Sub-tab dentro de Dashboard > Checkout. Vai conectar com Google Ads API
// pra trazer: campanhas Search, Display, YouTube, Performance Max, gasto,
// impressões, cliques, conversões, ROAS.
//
// Roadmap futuro (standby):
//   - OAuth Google Ads (developer token + MCC)
//   - GET /api/google-ads-dashboard-metrics
//   - GAQL queries pra campaign/ad_group/keyword
//   - Conversion tracking integrado com Hotmart purchase events
    ...
    return `<div class="space-y-4">
      ...
      <div class="rounded-3xl p-6 lg:p-8" style="background: linear-gradient(135deg, rgba(244,114,182,.18), rgba(249,168,212,.10)); border: 1px solid rgba(244,114,182,.30);">
        <div class="flex items-start gap-4">
          ...
          <div class="min-w-0 flex-1">
            ...
            <h2 class="text-2xl lg:text-3xl font-black text-slate-900">Google Ads</h2>
            <p class="text-sm text-slate-600 mt-2">Search, Display, YouTube, Performance Max. Quem clicou na palavra-chave e virou cliente Hotmart.</p>
          </div>
        </div>
      </div>
      ...
      <div class="rounded-3xl bg-white border-2 border-dashed p-8 text-center" style="border-color: rgba(244,114,182,.35);">
        ...
        <p class="text-sm text-slate-600 max-w-md mx-auto mb-6">
          Esta aba vai conectar com a <strong>Google Ads API</strong> pra trazer dados de campanha em tempo real. Disponível em breve.
        </p>
        ...
        <div class="max-w-2xl mx-auto text-left">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ${[
              { icon: 'dollar-sign',     title: 'Gasto + ROAS por campanha',  desc: 'Search, Display, YouTube, PMax. Quanto cada uma retornou.' },
              { icon: 'tag',              title: 'Performance por keyword',    desc: 'CTR, CPC, qualidade. Quais termos convertem mais.' },
              { icon: 'monitor-play',    title: 'YouTube + Display reach',    desc: 'Impressões, view rate, custo por view.' },
              { icon: 'flag',             title: 'Conversões cross-platform',  desc: 'Click no anúncio → compra Hotmart amarrada por tag automática.' }
        ...
        <div class="mt-6 inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest" style="background: rgba(244,114,182,.10); color: #F472B6;">
          ...
          Roadmap · Google Ads API standby
        </div>
      </div>
    </div>`;
  }
