var ActionModule = {
  ...
  form() {
    ...
    return `<div class="space-y-3">
      ...
      <div><label class="text-xs font-black text-slate-500">Nome da ação</label><input id="action-draft-name" data-focus-key="action-draft-name" value="${Utils.escape(d.name)}" oninput="App.state.actionDraft.name=this.value; App.save();" placeholder="Ex: Post orgânico Instagram" class="w-full px-4 py-3 rounded-2xl bg-slate-100 font-semibold" /></div>
  ...
  okrRow(okr, index) { return `<div class="grid grid-cols-[1fr_74px_74px_auto] gap-2"><input id="action-draft-okr-${index}-name" data-focus-key="action-draft-okr-${index}-name" value="${Utils.escape(okr.name)}" oninput="Actions.updateActionDraftOkr(${index}, 'name', this.value)" placeholder="Nome do KPI" class="w-full px-3 py-2.5 rounded-2xl bg-white border border-slate-200 font-semibold text-sm" /><input id="action-draft-okr-${index}-target" data-focus-key="action-draft-okr-${index}-target" value="${Utils.escape(okr.target || '')}" oninput="Actions.updateActionDraftOkr(${index}, 'target', this.value)" placeholder="Meta" class="w-full px-3 py-2.5 rounded-2xl bg-white border border-slate-200 font-black text-sm" /><input id="action-draft-okr-${index}-current" data-focus-key="action-draft-okr-${index}-current" value="${Utils.escape(okr.current || '')}" oninput="Actions.updateActionDraftOkr(${index}, 'current', this.value)" placeholder="Atual" class="w-full px-3 py-2.5 rounded-2xl bg-white border border-slate-200 font-black text-sm" /><button onclick="Actions.removeActionDraftOkr(${index})" class="px-3 py-2 rounded-2xl bg-white border border-slate-200 text-red-500 font-black">×</button></div>`; },
