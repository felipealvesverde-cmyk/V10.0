      ...
      <div class="flex flex-col gap-4">
        <div class="lj-entity-card-grid">
          ...
          <div class="flex flex-col items-end justify-end gap-2 min-w-[220px]">
            <!-- V38.1.55 — Botão "Ver Fluxo da Ação" + ActionFlowModal eliminados.
                 Roadmap aqui abre o roadmap da CAMPANHA (mesmo do botão do card de campanha). -->
            <button onclick="event.stopPropagation(); Actions.openCampaignFlowModal(${action.campaignId})" style="color:#fff!important; ${areaIsConnected ? '' : 'border-color: var(--lj-action);'}" class="w-full px-4 py-2.5 rounded-2xl bg-slate-900 text-white font-bold text-xs lj-dark-button border-2 ${areaIsConnected ? `border-${areaTone}-500` : ''} flex items-center justify-center gap-1.5"><i data-lucide="route" class="w-3.5 h-3.5"></i> Roadmap</button>
