var ProductsModule = {
  ...
  flowCampaignCard(campaign) {
    ...
    return `<button onclick="Actions.openCampaignFlowModal(${campaign.id})" class="w-full text-left min-h-[104px] rounded-[22px] p-4 bg-white/[0.04] border border-white/10 hover:bg-violet-500/10 hover:border-violet-400/40 transition"><div class="flex items-start justify-between gap-3 mb-2"><div><p class="font-black text-sm text-white">${Utils.escape(campaign.name)}</p><p class="text-xs text-slate-400 mt-1">${Utils.escape(campaign.status || 'Ativa')} • ${actions.length} ação(ões)</p></div><span class="text-xs px-2 py-1 rounded-full bg-white/10 border border-white/10">${leads} leads</span></div><div class="grid grid-cols-3 gap-2 text-xs mt-3"><div><p class="text-slate-500">Ações</p><p class="font-black">${actions.length}</p></div><div><p class="text-slate-500">Setor</p><p class="font-black">${Utils.escape(campaign.sector || 'Marketing')}</p></div><div><p class="text-slate-500">Fluxo</p><p class="font-black">Ver</p></div></div></button>`;
  },
