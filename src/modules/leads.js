    ...
    return `<div class="p-4 rounded-3xl bg-slate-50 border border-slate-100 hover:bg-slate-100 transition">
      <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div onclick="Actions.openLead('${safeId}')" class="cursor-pointer min-w-0 flex-1">
          <div class="flex items-center gap-2 mb-1 flex-wrap"><h3 class="font-black text-lg">${Utils.escape(lead.name)}</h3><span class="px-3 py-1 rounded-full text-xs font-black ${tempClass}">${temperature}</span>${audienceBadge}</div>
