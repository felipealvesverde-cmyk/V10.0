...
window.GoogleAdsWizardModal = {
  ...
  _step1Credentials(w) {
    const d = w.draft;
    return `<div class="space-y-4">
      <div class="rounded-xl bg-amber-500/5 border border-amber-400/30 p-3 space-y-2">
        <p class="text-[11px] font-black text-amber-200 uppercase tracking-widest">O que você vai precisar</p>
      ...
      <div class="flex justify-end gap-2 pt-1">
        ...
        <button onclick="Actions.saveGoogleAdsCredentials()" ${w.saving ? 'disabled' : ''} class="px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-xs font-black flex items-center gap-2" style="color:#fff;">
    ...
    return `<div class="space-y-4 text-center">
      ...
      <div>
        ...
        <p class="text-[12px] text-slate-300 mt-1">
          Conta <span class="font-mono text-amber-300">${Utils.escape(status.selectedCustomerId || '?')}</span> ligada ao LeadJourney.
        </p>
      </div>
