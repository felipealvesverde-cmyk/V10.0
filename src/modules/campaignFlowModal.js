var CampaignFlowModal = {
  ...
  icpMetric(leads, product) {
    ...
    const summary = (window.AudienceTransmutationEngine && hasAudience)
      ...
      : null;
    const header = `<div class="flex items-center justify-between gap-3 mb-3"><p class="text-xs text-slate-400 font-bold">ICP da Campanha</p><div class="w-7 h-7 rounded-xl bg-white/10 grid place-items-center text-violet-300"><i data-lucide="target" class="w-3.5 h-3.5"></i></div></div>`;
    if (!summary) {
      const msg = product
