...
function salesItemToWebhookPayload(item) {
  ...
  // V35.2.1 — Sales API pode trazer refusal_reason em vários nomes; passamos
  // adiante e o extractPurchaseData/normalizeReason cuidam de tudo.
  const purchase = item?.purchase || item || {};
  return {
    event: STATUS_TO_EVENT[status] || 'PURCHASE_APPROVED',
    creation_date: purchase?.approved_date || purchase?.order_date || null,
    data: {
      buyer: item?.buyer || {},
