...
async function runReconciliation(masterDb, tenantDb, userId, token, opts = {}) {
  ...
  // V34.9.4 — Processa pending-stage-update (mirror retry) e pending-deal-creation
  let stageRetrySynced = 0, stageRetryFailed = 0;
  let pendingDealsCreated = 0, pendingDealsCreateFailed = 0;
  try {
    const sr = await processPendingStageUpdates(masterDb, tenantDb, userId, token, { max: 100 });
    stageRetrySynced = sr.synced; stageRetryFailed = sr.failed;
    const dc = await processPendingDealCreation(masterDb, tenantDb, userId, token, { max: 50 });
    pendingDealsCreated = dc.created; pendingDealsCreateFailed = dc.failed;
  } catch (err) {
    console.warn('[runReconciliation] pending stage/deal processing falhou:', err.message);
  }
  ...
  return {
    ...
    pendingStage: { synced: stageRetrySynced, failed: stageRetryFailed },
    pendingDeal: { created: pendingDealsCreated, failed: pendingDealsCreateFailed },
...
// V34.9.4 — Processa visitors com pending-stage-update via mirrorStageToRd.
// Cron passa por aqui depois de tudo (push/orphans/deals).
async function processPendingStageUpdates(masterDb, tenantDb, userId, token, opts = {}) {
  const max = Math.min(Number(opts.max || 100), 500);
  const { mirrorStageToRd } = require('./rd-stage-mirror');
  let synced = 0, failed = 0;
  try {
    const r = await tenantDb.query(
      `SELECT v.lj_visitor_id, v.current_stage, v.external_rd_deal_id,
              v.external_rd_contact_id,
              (SELECT campaign_id FROM lj_visitor_campaign_state vcs
                WHERE vcs.user_id = v.user_id AND vcs.lj_visitor_id = v.lj_visitor_id
                ORDER BY updated_at DESC LIMIT 1) AS campaign_id
         FROM lj_visitors v
        WHERE v.user_id = $1
          AND v.external_rd_sync_status = 'pending-stage-update'
        ORDER BY v.updated_at DESC LIMIT $2`,
      [userId, max]
    );
    for (const visitor of r.rows) {
      if (!visitor.campaign_id) { failed++; continue; }
      const res = await mirrorStageToRd({
        tenantDb, userId, visitor,
        campaignId: visitor.campaign_id,
        newStage: visitor.current_stage, token
      });
      if (res.ok) synced++; else failed++;
    }
  } catch (err) {
    console.warn('[processPendingStageUpdates]', err.message);
  }
  return { synced, failed };
}

// V34.9.4 — Cria deal no RD pra visitors com pending-deal-creation.
// POST /deals com contato vinculado (vínculo aceita na criação inicial).
async function processPendingDealCreation(masterDb, tenantDb, userId, token, opts = {}) {
  const max = Math.min(Number(opts.max || 50), 200);
  const { resolveRdStageMapping } = require('./rd-stage-mirror');
  let created = 0, failed = 0;
  try {
    const r = await tenantDb.query(
      `SELECT v.lj_visitor_id, v.name, v.email, v.phone, v.current_stage,
              v.external_rd_contact_id,
              (SELECT campaign_id FROM lj_visitor_campaign_state vcs
                WHERE vcs.user_id = v.user_id AND vcs.lj_visitor_id = v.lj_visitor_id
                ORDER BY updated_at DESC LIMIT 1) AS campaign_id
         FROM lj_visitors v
        WHERE v.user_id = $1
          AND v.external_rd_sync_status = 'pending-deal-creation'
        ORDER BY v.updated_at DESC LIMIT $2`,
      [userId, max]
    );
    for (const v of r.rows) {
      // V34.9.8 — Variante O: passa dados do contato (sem id) — RD não
      // aceita id no contacts[]. Não exige external_rd_contact_id pré-existente.
      if (!v.campaign_id) { failed++; continue; }
      const mapping = await resolveRdStageMapping(tenantDb, userId, v.campaign_id, v.current_stage);
      if (!mapping) { failed++; continue; }
      if (!v.email && !v.phone) { failed++; continue; }

      const contactBlock = {
        name: v.name || v.email || 'Lead LJ',
      ...
      const dealBody = {
        deal: {
          name: v.name || v.email || 'Lead LJ',
          deal_stage_id: mapping.dealStageId,
          deal_pipeline_id: mapping.pipelineId
        },
        contacts: [contactBlock]
      };
      const rd = await rdFetch('/deals', token, { method: 'POST', body: dealBody });
      if (rd.ok && (rd.data?.id || rd.data?.deal?.id)) {
        const newDealId = String(rd.data.id || rd.data.deal.id);
        // Pega contact_id criado via GET /deals/{id}/contacts
        let contactId = v.external_rd_contact_id;
        try {
          const cResp = await rdFetch(`/deals/${encodeURIComponent(newDealId)}/contacts`, token);
          const cs = cResp.data?.contacts || cResp.data?.data || [];
          if (Array.isArray(cs) && cs.length) contactId = String(cs[0].id || cs[0]._id);
        } catch (_) {}
        await tenantDb.query(
          `UPDATE lj_visitors SET external_rd_deal_id = $3,
             external_rd_contact_id = COALESCE($4, external_rd_contact_id),
             external_rd_sync_status = 'synced', external_rd_sync_error = NULL,
             external_rd_synced_at = NOW(), updated_at = NOW()
            WHERE user_id = $1 AND lj_visitor_id = $2`,
          [userId, v.lj_visitor_id, newDealId, contactId || null]
        );
        created++;
      } else {
        failed++;
        await tenantDb.query(
          `UPDATE lj_visitors SET external_rd_sync_error = $3, updated_at = NOW()
            WHERE user_id = $1 AND lj_visitor_id = $2`,
          [userId, v.lj_visitor_id, `deal-create HTTP ${rd.status}`.slice(0, 200)]
        );
      }
      await sleep(200);
    }
  } catch (err) {
    console.warn('[processPendingDealCreation]', err.message);
  }
...
module.exports = {
  ...
  resolveLjVisitor,
  recordAlert,
  processPendingStageUpdates,
  processPendingDealCreation,
  COMPARED_FIELDS,
  CONFLICT_WINDOW_SEC
};
