...
// V34.9.0 — Vincula contato a deal no RD (caso deals existirem soltos).
// POST /deals/{deal_id}/contacts body { contacts: [{ id: contact_id }] }
async function linkContactToDeal(db, userId, ljVisitor, token) {
  ...
  const r = await rdFetch(`/deals/${encodeURIComponent(dealId)}/contacts`, token, {
    method: 'POST',
    body: { contacts: [{ id: contactId }] }
  });
  if (r.ok) {
    await db.query(
  ...
  // RD pode retornar 409/422 se já vinculado — tratar como sucesso idempotente
