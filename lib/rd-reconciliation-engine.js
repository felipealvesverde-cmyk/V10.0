      ...
      if (rd.ok && (rd.data?.id || rd.data?.deal?.id)) {
        ...
        await tenantDb.query(
          `UPDATE lj_visitors SET external_rd_deal_id = $3,
            ...
            WHERE user_id = $1 AND lj_visitor_id = $2`,
          [userId, v.lj_visitor_id, newDealId]
        );
        created++;
      } else {
        failed++;
