...
// Helper interno: garante tag `lj-auto` existe no space (idempotente).
async function ensureLjTag(db, userId, spaceId, tagName) {
  if (!spaceId || !tagName) return false;
  try {
    const list = await clickupFetch(db, userId, 'GET', `/space/${spaceId}/tag`);
...
async function prepareTaskBody({ db, userId, input, state }) {
  ...
  const useMirror = cred.mirror_enabled !== false && cred.lj_space_id && mirror_context
    && mirror_context.product?.id && mirror_context.campaign?.id && mirror_context.action?.id;

  if (useMirror) {
    try {
      const chain = await mirror.resolveActionChain(
        db, userId, cred.lj_space_id,
  ...
  if (!initialStatus && cred.status_map_json) {
    try {
      ...
      if (map?.pending) {
        ...
        try {
          const listMeta = await clickupFetch(db, userId, 'GET', `/list/${targetListId}`);
  ...
  // 6. Tag automática (com fallback lj_space_id — V32.2.2 A1)
