// V32.2.0 → V32.6.0 — Hierarquia espelhada LJ → ClickUp.
//
// V32.6.0 (raiz flexível): cliente escolhe ancorar LJ em Space, Folder ou List.
// O tipo da raiz determina o nível de cascading:
//
//   root.kind = 'space'  → cascado completo:
//     Space cliente > Folder(Produto) > List(Campanha) > Task(Ação) > Subtask(Tarefa)
//
//   root.kind = 'folder' → cascado parcial (Produto vira metadado LJ):
//     Folder cliente > List(Campanha) > Task(Ação) > Subtask(Tarefa)
//
//   root.kind = 'list'   → achatado (Produto/Campanha/Ação só no LJ):
//     List cliente > Task(Tarefa)     — sem subtasks por ação
//
// clickup_lj_mappings continua sendo (user_id, lj_kind, lj_id) → clickup_id, mas
// nos modos folder/list as rows correspondentes (product e/ou campaign+action)
// não são criadas pq não há entity ClickUp pra mapear.
//
// Find-or-create cascado: cada nível verifica se a entity ainda existe no ClickUp
...
// V32.2.4 (Geraldo A14) — Variant que faz SELECT FOR UPDATE pra prevenir race
// condition em find-or-create cascado. Quando 2 calls simultâneas pra mesma
// ação tentam criar folder/list/task pai, a segunda fica esperando a primeira
// commitar. Evita duplicatas.
//
// Uso: dentro de uma transação (db = pgClient com BEGIN ativo).
async function getMappingForUpdate(client, userId, ljKind, ljId) {
  const r = await client.query(
    'SELECT clickup_id, clickup_kind, clickup_name FROM clickup_lj_mappings WHERE user_id = $1 AND lj_kind = $2 AND lj_id = $3 FOR UPDATE',
    [userId, ljKind, Number(ljId)]
  );
  return r.rows[0] || null;
}
...
// V32.6.0 — Resolve cadeia respeitando o tipo da raiz.
//
// `root` = { id, kind }, kind ∈ ('space'|'folder'|'list').
//
// Retorna sempre { listId, actionParentTaskId, folderId?, createdAny, mode }
// onde:
//   - listId = onde a Task vai ser criada (POST /list/{listId}/task)
//   - actionParentTaskId = `parent` da Task (null se modo flat)
//   - folderId = só preenchido em modo space (compat)
//   - mode = root.kind reutilizado pelo caller pra logs/UI
async function resolveActionChain(db, userId, root, product, campaign, action) {
  // Back-compat: caller antigo passava `ljSpaceId` (string) direto.
  // Trata como { id: ljSpaceId, kind: 'space' }.
  const r = (root && typeof root === 'object' && 'kind' in root)
    ? root
    : { id: String(root || ''), kind: 'space' };

  if (!r.id) throw new Error('raiz LJ (root) não configurada — rode o setup wizard.');
  if (!['space', 'folder', 'list'].includes(r.kind)) {
    throw new Error(`root.kind inválido: ${r.kind}`);
  }

  if (!action?.id) throw new Error('action.id obrigatório');

  // ── MODO LIST (achatado) ──────────────────────────────────────
  // Toda Tarefa LJ vira Task direto na list raiz. Sem parent, sem
  // mapping de product/campaign/action no ClickUp.
  if (r.kind === 'list') {
    return {
      folderId: null,
      listId: r.id,
      actionParentTaskId: null,
      createdAny: false,
      mode: 'list'
    };
  }

  // Para folder/space ação precisa de campaign
  if (!campaign?.id) throw new Error('campaign.id obrigatório nos modos space/folder');

  // ── MODO FOLDER (cascado parcial) ─────────────────────────────
  // Folder do cliente é a raiz. LJ cria List(Campanha) lá dentro,
  // Task(Ação) na List e Subtask(Tarefa) sob a Action. Produto LJ
  // não tem espelho — fica só como metadado no LJ.
  if (r.kind === 'folder') {
    const campaignRes = await ensureCampaignList(db, userId, r.id, campaign.id, campaign.name);
    ...
    return {
      ...
      createdAny: campaignRes.created || actionRes.created,
      mode: 'folder'
    };
  }

  // ── MODO SPACE (cascado completo) ─────────────────────────────
  // Mesmo comportamento legado V32.2.x: Folder(Produto) > List(Campanha) > Task(Ação).
  if (!product?.id) throw new Error('product.id obrigatório no modo space');

  const productRes = await ensureProductFolder(db, userId, r.id, product.id, product.name);
...
module.exports = {
  getMapping,
  getMappingForUpdate,  // V32.2.4 A14
  setMapping,
  deleteMapping,
  listMappings,
  verifyClickupEntity,
