...
// =====================================
// V34.9.11 — ICP Fit Score: % de match do visitor com perfil ideal cadastrado.
// Lê lj_icp_profile do user, compara campos do visitor com fields_json.
// Retorna { fit_percentage: 0-100, matchedFields: [...], totalFields: N, breakdown: [...] }.
// =====================================
async function computeFitScore(db, userId, visitor) {
  try {
    const r = await db.query(
      `SELECT fields_json, tier_method, tier_rules_json FROM lj_icp_profile WHERE user_id = $1`,
      [userId]
    );
    if (!r.rows.length) return { fit_percentage: 0, tier: null, tierMethod: 'percentage', matchedFields: 0, totalFields: 0, breakdown: [], rulesTrace: null, profile: null };

    const profile = r.rows[0];
    const tierMethod = profile.tier_method || 'percentage';
    const fields = profile.fields_json || {};
    const breakdown = [];
    let matched = 0, total = 0;

    // Compara cada campo do ICP com o visitor (sempre roda — métricas % servem como overview mesmo em modo rules)
    for (const [key, expected] of Object.entries(fields)) {
      if (expected === null || expected === undefined) continue;
      if (key === 'idade_min' || key === 'idade_max') continue;
      total++;
      const visitorValue = visitor[key];
      if (visitorValue === undefined || visitorValue === null || visitorValue === '') {
        breakdown.push({ field: key, expected, actual: null, match: false });
        continue;
      }
      let match = false;
      if (Array.isArray(expected)) {
        match = expected.map(x => String(x).toLowerCase()).includes(String(visitorValue).toLowerCase());
      } else {
        match = String(expected).toLowerCase() === String(visitorValue).toLowerCase();
      }
      if (match) matched++;
      breakdown.push({ field: key, expected, actual: visitorValue, match });
    }

    if (fields.idade_min !== undefined || fields.idade_max !== undefined) {
      total++;
      const age = Number(visitor.idade || 0);
      const minOk = fields.idade_min == null || age >= Number(fields.idade_min);
      const maxOk = fields.idade_max == null || age <= Number(fields.idade_max);
      const match = age > 0 && minOk && maxOk;
      if (match) matched++;
      breakdown.push({ field: 'idade', expected: `${fields.idade_min ?? '–'}–${fields.idade_max ?? '–'}`, actual: age || null, match });
    }

    const fit_percentage = total > 0 ? Math.round((matched / total) * 100) : 0;

    let tier, rulesTrace = null;
    if (tierMethod === 'rules') {
      const evalResult = evalTierRules(profile.tier_rules_json, visitor);
      tier = evalResult.tier;
      rulesTrace = evalResult.trace;
    } else {
      tier = fitTierFromPercentage(fit_percentage, total);
    }

    return { fit_percentage, tier, tierMethod, matchedFields: matched, totalFields: total, breakdown, rulesTrace, profile };
  } catch (err) {
    console.warn('[computeFitScore]', err.message);
    return { fit_percentage: 0, tier: null, tierMethod: 'percentage', matchedFields: 0, totalFields: 0, breakdown: [], rulesTrace: null, profile: null };
  }
}

// V34.9.13 — Avalia regras de tier (modo HubSpot puro).
// Estrutura: { tier_1: [[cond, cond], [cond]], tier_2: [...], tier_3: [...] }
// Cada tier tem grupos AND. Tier vence se qualquer grupo tiver TODAS condições verdadeiras.
// Ordem: Tier 1 → Tier 2 → Tier 3. Se nenhum casa → tier null.
function evalTierRules(rules, visitor) {
  if (!rules || typeof rules !== 'object') return { tier: null, trace: { reason: 'sem_regras' } };
  for (const tier of [1, 2, 3]) {
    const groups = rules[`tier_${tier}`] || [];
    if (!Array.isArray(groups) || groups.length === 0) continue;
    for (let gi = 0; gi < groups.length; gi++) {
      const group = groups[gi];
      if (!Array.isArray(group) || group.length === 0) continue;
      const conds = group.map(c => ({ cond: c, match: matchCondition(c, visitor) }));
      if (conds.every(x => x.match)) {
        return { tier, trace: { matchedTier: tier, matchedGroupIndex: gi, conditions: conds } };
      }
    }
  }
  return { tier: null, trace: { reason: 'nenhuma_regra_casou' } };
}

function matchCondition(cond, visitor) {
  if (!cond || !cond.field) return false;
  const v = visitor[cond.field];
  ...
  switch (op) {
    ...
    case '==':
    case 'equals':
      return vStr === expStr && v !== undefined && v !== null && v !== '';
    case '≠':
    case '!=':
    case 'not_equals':
      return vStr !== expStr;
    case 'in':
      return Array.isArray(expected) && expected.some(e => String(e).toLowerCase() === vStr);
    case 'not_in':
      return Array.isArray(expected) && !expected.some(e => String(e).toLowerCase() === vStr);
    case '>': return Number(v) > Number(expected);
    case '<': return Number(v) < Number(expected);
    case '>=': return Number(v) >= Number(expected);
    case '<=': return Number(v) <= Number(expected);
    case 'between':
      return Array.isArray(expected) && expected.length === 2 &&
             Number(v) >= Number(expected[0]) && Number(v) <= Number(expected[1]);
    default: return false;
  }
}

// V34.9.12 — Mapeia fit_percentage em tier (HubSpot-style).
// Faixas fixas no código: 80%+ Tier 1, 50%+ Tier 2, <50% Tier 3.
// Se totalFields = 0 (perfil ICP não configurado) → tier = null.
function fitTierFromPercentage(pct, totalFields) {
  if (!totalFields || totalFields === 0) return null;
  if (pct >= 80) return 1;
  if (pct >= 50) return 2;
  return 3;
}
...
// =====================================
// V34.9.10.6 — Modelo "Critérios" (HubSpot puro): soma de pontos direta.
// Retorna pontos brutos, agrupados por categoria. Sem normalização, sem
// clamp hierárquico. Score do visitor = totalPoints diretos.
// =====================================
async function computeCriteriaScore(db, userId, visitor, signals) {
  let totalPoints = 0;
  let hits = 0;
  const breakdown = [];
  const byCategory = { engagement: 0, fit: 0, intent: 0, uncategorized: 0 };
  try {
    const r = await db.query(
    ...
    for (const rule of r.rows) {
      ...
      if (matched) {
        ...
        byCategory[cat] = (byCategory[cat] || 0) + points;
        breakdown.push({ ruleId: rule.id, type, param, points, category: rule.category });
      }
    }
  } catch (err) {
    console.warn('[computeCriteriaScore]', err.message);
  }
  // V34.9.12 — HubSpot-puro: engagement score e ICP fit ficam SEPARADOS.
  // Score = totalPoints (engagement direto). Tier do fit é indicador paralelo.
  const fit = await computeFitScore(db, userId, visitor);
  return { score: totalPoints, totalPoints, hits, breakdown, byCategory, fit };
}
...
async function resolveScoreByModel({ model, visitor, signals, daysSinceLastEvent, totalEvents, db, userId, campaignState = null }) {
  if (model === 'criteria') {
    // V34.9.10.6 — HubSpot puro: pontos diretos, sem clamp hierárquico, sem normalização.
    const c = await computeCriteriaScore(db, userId, visitor, signals);
    return { score: c.totalPoints, model: 'criteria', criteria: c, rfv: null };
  }
