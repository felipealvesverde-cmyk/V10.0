...
// =====================================
async function applyEvent(db, userId, visitorId, eventInfo = {}) {
  // Lê visitor
  ...
  const globalResult = computeScore({ visitor, campaignState: null, signals, daysSinceLastEvent, totalEvents });
  ...
  for (const cs of csRes.rows) {
    const campResult = computeScore({ visitor, campaignState: cs, signals, daysSinceLastEvent, totalEvents });
    // Score campanha = score base × 1.0 (campanha tem peso integral, conforme cravado)
    await db.query(
    ...
    campaignResults.push({ campaignId: cs.campaign_id, ...campResult });
  }
