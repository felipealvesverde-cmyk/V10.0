...
async function computeFitScore(db, userId, visitor) {
  try {
    ...
    const tier = fitTierFromPercentage(fit_percentage, total);
    return { fit_percentage, tier, matchedFields: matched, totalFields: total, breakdown, profile };
  } catch (err) {
    console.warn('[computeFitScore]', err.message);
    return { fit_percentage: 0, tier: null, matchedFields: 0, totalFields: 0, breakdown: [], profile: null };
  }
}
