...
async function computeFitScore(db, userId, visitor) {
  try {
    const r = await db.query(
      `SELECT fields_json, scoring_method, fit_max_bonus FROM lj_icp_profile WHERE user_id = $1`,
      [userId]
    );
    if (!r.rows.length) return { fit_percentage: 0, matchedFields: [], totalFields: 0, breakdown: [], profile: null };
    ...
    // Compara cada campo do ICP com o visitor
    for (const [key, expected] of Object.entries(fields)) {
      if (expected === null || expected === undefined) continue;
      // Range fields: idade_min / idade_max
      if (key === 'idade_min' || key === 'idade_max') continue; // tratado abaixo
      total++;
    ...
    return { fit_percentage, matchedFields: matched, totalFields: total, breakdown, profile };
  } catch (err) {
    console.warn('[computeFitScore]', err.message);
    return { fit_percentage: 0, matchedFields: 0, totalFields: 0, breakdown: [], profile: null };
  }
}
...
async function computeCriteriaScore(db, userId, visitor, signals) {
  ...
  // V34.9.11 — Aplica ICP Fit conforme scoring_method do user
  const fit = await computeFitScore(db, userId, visitor);
  const method = fit.profile?.scoring_method || 'multiplier';
  const fitMaxBonus = Number(fit.profile?.fit_max_bonus || 100);
  const engagement = totalPoints;
  let finalScore = engagement;
  let fitBonus = 0;
  if (fit.totalFields > 0) {
    if (method === 'multiplier') {
      // Engagement × (1 + Fit/100)
      finalScore = Math.round(engagement * (1 + fit.fit_percentage / 100));
      fitBonus = finalScore - engagement;
    } else if (method === 'sum') {
      // Engagement + (Fit% × max_bonus / 100)
      fitBonus = Math.round((fit.fit_percentage / 100) * fitMaxBonus);
      finalScore = engagement + fitBonus;
    } // 'simple' → finalScore = engagement (sem fit)
  }

  return { score: finalScore, totalPoints: finalScore, engagement, fitBonus, hits, breakdown, byCategory, fit };
}
