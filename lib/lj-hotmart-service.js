...
// V35.2.1 — Mapeamento granular de motivos de recusa de cartão.
// Quando PURCHASE_CANCELED traz refusal_reason no payload, tagamos
// granularmente. Vendas BOF ganha visibilidade sobre POR QUÊ perdeu.
// Cobertura: top 7 motivos comuns + fallback 'outros'.
const REASON_MAP = {
  INVALID_CARD_NUMBER:       { tag: 'lj-recusa-cartao-invalido',   label: 'Cartão inválido' },
  CARD_EXPIRED:              { tag: 'lj-recusa-cartao-vencido',    label: 'Cartão vencido' },
  INSUFFICIENT_FUNDS:        { tag: 'lj-recusa-sem-saldo',         label: 'Sem saldo' },
  ISSUING_BANK_UNAVAILABLE:  { tag: 'lj-recusa-banco-fora',        label: 'Banco fora do ar' },
  CARD_DECLINED_BY_BANK:     { tag: 'lj-recusa-banco-negou',       label: 'Banco negou' },
  FRAUD_SUSPICION:           { tag: 'lj-recusa-fraude',            label: 'Suspeita de fraude' },
  BLOCKED_CARD:              { tag: 'lj-recusa-cartao-bloqueado',  label: 'Cartão bloqueado' }
};
const REASON_OTHERS = { tag: 'lj-recusa-outros', label: 'Outros motivos' };

function normalizeReason(raw) {
  if (!raw) return null;
  const s = String(raw).toUpperCase().trim();
  // Hotmart pode mandar variações; tenta mapear pelos prefixos conhecidos.
  if (REASON_MAP[s]) return s;
  if (s.includes('EXPIRED') || s.includes('VENCID')) return 'CARD_EXPIRED';
  if (s.includes('INSUFFICIENT') || s.includes('SALDO')) return 'INSUFFICIENT_FUNDS';
  if (s.includes('FRAUD')) return 'FRAUD_SUSPICION';
  if (s.includes('BLOCKED') || s.includes('BLOQUEAD')) return 'BLOCKED_CARD';
  if (s.includes('INVALID') && s.includes('CARD')) return 'INVALID_CARD_NUMBER';
  if (s.includes('DECLINED') || s.includes('NEGAD')) return 'CARD_DECLINED_BY_BANK';
  if (s.includes('UNAVAILABLE') || s.includes('INDISPON')) return 'ISSUING_BANK_UNAVAILABLE';
  return s; // mantém raw — vira "Outros" no breakdown
}

function reasonToTag(normalized) {
  if (!normalized) return null;
  return REASON_MAP[normalized] || REASON_OTHERS;
}
...
function extractPurchaseData(payload) {
  ...
  // V35.2.1 — Motivo de recusa quando PURCHASE_CANCELED
  const rawReason = purchase?.payment?.refusal_reason
                 || purchase?.payment?.refusalReason
                 || purchase?.refusal_reason
                 || purchase?.status_reason
                 || null;
  const cancellationReason = normalizeReason(rawReason);
...
async function processWebhook({ tenantDb, userId, payload, productMappings }) {
  ...
  // V35.2.1 — persiste cancellation_reason (NULL pra eventos não-cancelamento)
  await tenantDb.query(
    `INSERT INTO lj_hotmart_purchases
     ...
     ON CONFLICT (user_id, transaction_id) DO UPDATE SET
       ...
       cancellation_reason = COALESCE(EXCLUDED.cancellation_reason, lj_hotmart_purchases.cancellation_reason),
  ...
  // V35.2.1 — Tag granular do motivo de cancelamento (cartão vencido, sem saldo, etc)
  if (visitor && data.cancellationReason && data.event === 'PURCHASE_CANCELED') {
    const reasonMeta = reasonToTag(data.cancellationReason);
    if (reasonMeta?.tag) {
      await applyHotmartTag(tenantDb, userId, visitor.lj_visitor_id, reasonMeta.tag, 'hotmart_webhook', 'hotmart-recusa');
    }
  }
...
module.exports = { processWebhook, extractPurchaseData, applyHotmartTag, EVENT_MAP, REASON_MAP, REASON_OTHERS };
