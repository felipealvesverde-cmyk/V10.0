...
// V37.4.28 — Email pra trocar email (owner solicita troca pro membro).
// O membro recebe no ENDEREÇO ATUAL — confirma que é ele que ainda controla
// o email atual ANTES de informar o novo na página de aceite.
function emailChangeRequestEmail({ requesterName, tenantName, actionUrl, currentEmail }) {
  const subject = `LeadJourney — Solicitação de troca de email`;
  const html = shellHtml(`
    <h1 style="margin:0 0 16px;font-size:22px;font-weight:900;color:${COLORS.text};line-height:1.3;">
      Troca de email solicitada
    </h1>
    <p style="margin:0 0 12px;font-size:14px;line-height:1.6;color:${COLORS.text};">
      <strong>${escapeHtml(requesterName)}</strong>, admin do tenant <strong>${escapeHtml(tenantName)}</strong>, solicitou uma troca de email pra sua conta.
    </p>
    <p style="margin:0 0 12px;font-size:14px;line-height:1.6;color:${COLORS.text};">
      Email atual: <strong>${escapeHtml(currentEmail)}</strong>
    </p>
    <p style="margin:0 0 24px;font-size:14px;line-height:1.6;color:${COLORS.muted};">
      Clique abaixo pra informar o novo email e confirmar com sua senha atual:
    </p>
    <p style="margin:0 0 24px;">
      <a href="${actionUrl}" style="display:inline-block;background:${COLORS.cta};color:#fff;text-decoration:none;font-weight:700;padding:14px 28px;border-radius:10px;font-size:14px;">
        Trocar meu email
      </a>
    </p>
    <p style="margin:0 0 8px;font-size:12px;color:${COLORS.muted};line-height:1.5;">
    ...
    <p style="margin:0;font-size:12px;color:${COLORS.muted};line-height:1.5;">
      Se você não pediu, ignore este email — sua conta segue protegida.
    </p>
  `);
  const text = `${requesterName}, admin de ${tenantName}, solicitou uma troca de email pra sua conta (${currentEmail}).\n\nTrocar em: ${actionUrl}\n\nSe você não pediu, ignore.`;
  return { subject, html, text };
}

module.exports = { inviteEmail, recoveryEmail, emailChangeRequestEmail };
