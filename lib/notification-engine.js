...
// V37.4.18 — controlPlaneDb separado do tenant db.
// tenant_members vive no CONTROL PLANE (users + tenants), não no tenant DB.
// Query precisa rodar no controlPlaneDb (req.db), não no req.tenantDb.
async function expandAudience(controlPlaneDb, tenantId, audience) {
  ...
  if (audience === 'tenant_wide') {
    const r = await controlPlaneDb.query('SELECT user_id FROM tenant_members WHERE tenant_id = $1', [tenantId]);
    return r.rows.map(row => Number(row.user_id));
  }
  if (typeof audience === 'object' && audience.role) {
    const r = await controlPlaneDb.query('SELECT user_id FROM tenant_members WHERE tenant_id = $1 AND LOWER(role) = LOWER($2)', [tenantId, audience.role]);
    return r.rows.map(row => Number(row.user_id));
  }
  return [];
}
...
async function createNotification({
  ...
  // V37.4.18 — expandAudience precisa do controlPlaneDb pra ler tenant_members.
  // Fallback pro db (tenant DB) só se controlPlaneDb não foi passado E audience
  // é user_id direto/array (não precisa de tenant_members).
  const audienceDb = controlPlaneDb || db;
  const userIds = await expandAudience(audienceDb, tenantId, audience);
