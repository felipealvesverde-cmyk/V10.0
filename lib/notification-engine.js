...
async function expandAudience(db, tenantId, audience) {
  ...
  if (typeof audience === 'object' && audience.role) {
    // V37.4.13 — Case-insensitive match. Roles em tenant_members podem estar
    // com maiúsculas/minúsculas variadas (legado V32).
    const r = await db.query('SELECT user_id FROM tenant_members WHERE tenant_id = $1 AND LOWER(role) = LOWER($2)', [tenantId, audience.role]);
    return r.rows.map(row => Number(row.user_id));
  }
  return [];
}
