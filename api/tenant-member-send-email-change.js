// V37.4.28 — POST /api/tenant-member-send-email-change
// Owner solicita troca de email pra um membro. Email vai pro ENDEREÇO ATUAL —
// só quem ainda tem acesso a ele consegue completar a troca. Na página, o
// membro informa o NOVO email + senha atual pra confirmar.
//
// Body: { userId }
//
// Cria token action_type='email_change', expira em 7d.
...
const { emailChangeRequestEmail } = require('../lib/email-templates');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const tenant = await req.db.query('SELECT name FROM tenants WHERE id = $1', [tenantId]);
    const tenantName = tenant.rows[0]?.name || 'seu tenant';

    const inviter = await req.db.query('SELECT display_name, username FROM users WHERE id = $1', [req.user.sub]);
    const requesterName = inviter.rows[0]?.display_name || inviter.rows[0]?.username || 'Admin';
    ...
    await req.db.query(
      `UPDATE user_action_tokens SET used_at = NOW()
       WHERE user_id = $1 AND action_type = 'email_change' AND used_at IS NULL`,
      [targetUserId]
    );

    await req.db.query(
      `INSERT INTO user_action_tokens (user_id, action_type, token, payload, issued_by_user_id, expires_at)
       VALUES ($1, 'email_change', $2, '{}'::jsonb, $3, $4)`,
      [targetUserId, token, req.user.sub, expiresAt]
    );

    const actionUrl = buildActionUrl(req, token);
    const tpl = emailChangeRequestEmail({ requesterName, tenantName, actionUrl, currentEmail: u.email });
